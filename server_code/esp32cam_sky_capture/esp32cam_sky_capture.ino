...
void loop() {
  ...
  if ((int32_t)(now - nextCaptureAt) < 0) {
    delay(2);
    return;
  }
  // Advance the deadline before blocking work so processing is inside 30 s.
  nextCaptureAt += CAPTURE_INTERVAL_MS;

  if (!serverAvailable &&
      (now - lastDiscoveryAttempt >= 5000 || lastDiscoveryAttempt == 0)) {
    lastDiscoveryAttempt = now;
    serverAvailable = discoverServer();
  }

  if (serverAvailable) {
    // Capture immediately at the fixed deadline. Configuration fetched here
    // is applied afterward and takes effect on the next scheduled capture.
    handleRackCommand(captureAndUpload());
    if (serverAvailable && fetchConfig()) applyCameraSettings();
  } else {
    Serial.println("Server not found; holding and retrying discovery.");
    handleRackCommand("hold");
  }

  const uint32_t finished = millis();
  while ((int32_t)(finished - nextCaptureAt) >= 0) {
    nextCaptureAt += CAPTURE_INTERVAL_MS;
    Serial.println("Capture slot skipped because processing exceeded 30 seconds.");
  }
}
